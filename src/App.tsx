...
import {
  ...
  ArrowUpRight, ArrowDownLeft, Menu, LogOut, ChevronRight, X
} from 'lucide-react';
import {
...
const [editingBookName, setEditingBookName] = useState('');
  ...
  if (window.confirm('Yakin hapus SEMUA data dari cloud?')) {
    await Promise.all([
      supabase.from('transactions').delete().neq('id', ''),
      supabase.from('budgets').delete().neq('id', ''),
      supabase.from('financial_goals').delete().neq('id', ''),
      supabase.from('recurring_transactions').delete().neq('id', ''),
    ]);
...
if (!activeBook) {
  return (
    <div className="min-h-[100dvh] bg-slate-900 flex items-center justify-center">
      <div className="text-center">
        <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-3" />
        <p className="text-white text-sm">Memuat buku...</p>
      </div>
    </div>
  );
}
